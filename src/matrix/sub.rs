        ...
        + Default,
{
    ...
    if total_elements >= SIMD_THRESHOLD && T::has_simd_support() {
        unsafe { sub_simd(m1, m2) }
    } else {
        sub_scalar(m1, m2)
    }
}

fn sub_scalar<T>(m1: &Matrix<T>, m2: &Matrix<T>) -> Matrix<T>
where
...
fn sub_assign_matrix_impl<T>(m1: &mut Matrix<T>, m2: &Matrix<T>)
where
    T: Clone
        + Add<Output = T>
        + Sub<Output = T>
        ...
        + Sync,
{
    ...
    // let total_elements = m1.rows * m1.cols;
    // TODO: implement sub assign simd
    sub_assign_scalar(m1, m2);
}

fn sub_assign_scalar<T>(m1: &mut Matrix<T>, m2: &Matrix<T>)
where
    T: Clone + Add<Output = T> + Sub<Output = T> + Mul<Output = T> + SubAssign<T> + Default,
{
    m1.data
        .iter_mut()
        .zip(&m2.data)
        ...
        + Sync,
{
    fn sub_assign(&mut self, other: Matrix<T>) {
        ...
        sub_assign_matrix_impl(self, &other);
    }
}
...
mod tests {
    ...
    #[test]
    fn test_sub_complex_matrix_refs() {
        ...
        assert_eq!(m3.rows, 3);
        assert_eq!(m3.cols, 3);
        assert_eq!(m3[(0, 0)], c32::new(0.0, 0.0)); // 2 - 2 = 0
        assert_eq!(m3[(1, 1)], c32::new(0.0, 0.0)); // 2 - 2 = 0
        assert_eq!(m3[(2, 2)], c32::new(0.0, 0.0)); // 2 - 2 = 0
    }

    #[test]
    fn test_sub_assign_matrices() {
    ...
    fn test_sub_large_matrix_c32() {
        ...
        assert_eq!(m3[(0, 0)], c32::new(-1.0, 0.0));
        assert_eq!(m3[(size - 1, size - 1)], c32::new(-1.0, 0.0));
        ...
        for _ in 0..100 {
            ...
            assert_eq!(m3[(row, col)], c32::new(-1.0, 0.0));
        }
    }

    #[test]
    fn test_sub_large_matrix_c64() {
        let size = 1024;
        ...
        assert_eq!(m3.rows, size);
        assert_eq!(m3.cols, size);
        assert_eq!(m3[(0, 0)], c64::new(-1.0, 0.0));
        assert_eq!(m3[(size - 1, size - 1)], c64::new(-1.0, 0.0));
        ...
        for _ in 0..100 {
            let row = rng.gen_range(0..size);
            let col = rng.gen_range(0..size);
            assert_eq!(m3[(row, col)], c64::new(-1.0, 0.0));
        }
    }
}
