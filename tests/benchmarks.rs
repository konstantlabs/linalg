...
pub fn bench_2x2_matrix_subs(c: &mut Criterion) {
    let mut group = c.benchmark_group("2x2 Matrix Subtraction");
...
pub fn bench_3x3_matrix_subs(c: &mut Criterion) {
    let mut group = c.benchmark_group("3x3 Matrix Subtraction");
...
pub fn bench_1024x1024_matrix_subs(c: &mut Criterion) {
    let mut group = c.benchmark_group("1024x1024 Matrix Subtraction");
    use rand::{thread_rng, Rng};
    let mut rng = thread_rng();
...
pub fn bench_2048x2048_matrix_adds(c: &mut Criterion) {
    let mut group = c.benchmark_group("2048x2048 Matrix Addition");
    use rand::{thread_rng, Rng};
    let mut rng = thread_rng();

    // f32
    {
        let data1: Vec<f32> = (0..2048 * 2048)
            .map(|_| rng.gen_range(-100.0..100.0))
            .collect();
        let data2: Vec<f32> = (0..2048 * 2048)
            .map(|_| rng.gen_range(-100.0..100.0))
            .collect();

        let m1: Matrix<f32> = Matrix::from_vec(2048, 2048, data1);
        let m2: Matrix<f32> = Matrix::from_vec(2048, 2048, data2);
...
pub fn bench_2048x2048_matrix_subs(c: &mut Criterion) {
    let mut group = c.benchmark_group("2048x2048 Matrix Subtraction");
    use rand::{thread_rng, Rng};
    let mut rng = thread_rng();
    ...
    // f64
    {
        let data1: Vec<f64> = (0..2048 * 2048)
            .map(|_| rng.gen_range(-100.0..100.0))
            .collect();
        let data2: Vec<f64> = (0..2048 * 2048)
            .map(|_| rng.gen_range(-100.0..100.0))
            .collect();

        let m1: Matrix<f64> = Matrix::from_vec(2048, 2048, data1);
        let m2: Matrix<f64> = Matrix::from_vec(2048, 2048, data2);
    ...
    // i32
    {
        let data1: Vec<i32> = (0..2048 * 2048).map(|_| rng.gen_range(-100..100)).collect();
        let data2: Vec<i32> = (0..2048 * 2048).map(|_| rng.gen_range(-100..100)).collect();

        let m1: Matrix<i32> = Matrix::from_vec(2048, 2048, data1);
        let m2: Matrix<i32> = Matrix::from_vec(2048, 2048, data2);
    ...
    // c32
    {
        let data1: Vec<c32> = (0..2048 * 2048)
            .map(|_| c32::new(rng.gen_range(-100.0..100.0), rng.gen_range(-100.0..100.0)))
            .collect();
        let data2: Vec<c32> = (0..2048 * 2048)
            .map(|_| c32::new(rng.gen_range(-100.0..100.0), rng.gen_range(-100.0..100.0)))
            .collect();

        let m1: Matrix<c32> = Matrix::from_vec(2048, 2048, data1);
        let m2: Matrix<c32> = Matrix::from_vec(2048, 2048, data2);
    ...
    // c64
    {
        let data1: Vec<c64> = (0..2048 * 2048)
            .map(|_| c64::new(rng.gen_range(-100.0..100.0), rng.gen_range(-100.0..100.0)))
            .collect();
        let data2: Vec<c64> = (0..2048 * 2048)
            .map(|_| c64::new(rng.gen_range(-100.0..100.0), rng.gen_range(-100.0..100.0)))
            .collect();

        let m1: Matrix<c64> = Matrix::from_vec(2048, 2048, data1);
        let m2: Matrix<c64> = Matrix::from_vec(2048, 2048, data2);
...
pub fn bench_2x2_matrix_muls(c: &mut Criterion) {
    let mut group = c.benchmark_group("2x2 Matrix Multiplication");
...
pub fn bench_1024x1024_matrix_muls(c: &mut Criterion) {
    let mut group = c.benchmark_group("1024x1024 Matrix Multiplication");
    use rand::{thread_rng, Rng};
    let mut rng = thread_rng();
...
criterion_group! {
    name = small_matrix_subtraction;
    config = Criterion::default().measurement_time(Duration::from_secs(10));
    targets =
        bench_2x2_matrix_subs,
        bench_3x3_matrix_subs,
}

criterion_group! {
    name = small_matrix_multiplication;
    config = Criterion::default().measurement_time(Duration::from_secs(10));
    targets =
        bench_2x2_matrix_muls,
        bench_3x3_matrix_muls,
}

criterion_group! {
    name = large_matrix_addition;
    config = Criterion::default().measurement_time(Duration::from_secs(30));
    targets =
        bench_1024x1024_matrix_adds,
        bench_1024x1024_matrix_subs,
        bench_2048x2048_matrix_adds,
        bench_2048x2048_matrix_subs,
}

criterion_group! {
    name = large_matrix_multiplication;
    config = Criterion::default().measurement_time(Duration::from_secs(30));
    targets =
        bench_1024x1024_matrix_muls
}

criterion_main!(
    small_matrix_addition,
